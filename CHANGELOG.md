...
- Added optional `externalAudioStream` parameter to `startListening()`, allowing callers to supply
  their own audio source (e.g. a VoIP call or a file) instead of the device
  microphone [Alfredo Bautista]<br>The
  stream must provide raw PCM audio: 16 kHz sample rate, mono channel, 16-bit little-endian — the
  same format the microphone produces<br>When `externalAudioStream` is `null`, behavior is identical
  to previous
  versions
...
- Language codes are now automatically sanitized to `languageCode-countryCode` format (e.g., "
  en" → "en-US")
...
- It is now possible to use short-lived token instead of subscription key using the "
  authorizationToken" parameter
